...
observe({
  ...
  htmldata <- reactive({
    ...
    html_test_create(input$html$datapath,
  ...
  maximum <- reactive({
    if (datatest() == TRUE) {
      NA
    } else if ("year" %in% names(excel_test()) && "category" %in% names(excel_test())) {
      # starting with the unpivoted excel data
      excel_test() |>
