...
### `View()`
```{r view, eval=FALSE}
View(kreft_html_with_variables)
View(kreft_excel)
```
...
testplot
ggplotly(testplot) |> layout(title = list(text = str_c('US State Population and Life Expectancy',
                                    '<br>',
                                    '<sup>',
                                    string,
                                    '</sup>'),
...
# function extracted
  ...
  # Reactive function to check for currency and use an appropriate subtitle
  currency_subtitle <- reactive({
...
ggplotly(expenses_plot) |> 
layout(annotations = plotlylayout())
        ...
        KPI_admin =
          str_c( # Using str_c() to concatenate value and % sign
            format( # shortening(lengthening) the resulting value
              ...
              (expenses_administrative / (expenses_program_services +
...
KPI_to_string(KPI_admin_num)

```

### Private donation growth
```{r}
kreft_excel |> 
  transmute(
...
```{r}
# private contributions, investments, cash in bank / short-term garnts payable, revocable endowments, long-term grants payable
...
kreft_excel |> 
  transmute(
      ...
      geom_col() +
      # Flipping, because the Excel file only contains two years atm
      coord_flip() +
...
```{r}
NGO <- read_excel("Data/OPXfinans_final.xlsx") |> 
  pull(Firm) |>
  first()
...
NGO_blank <- read_excel("Data/Bad_data_no_name.xlsx") |> 
  pull(Firm) |>
  first()
