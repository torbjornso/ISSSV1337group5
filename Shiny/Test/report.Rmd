...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, fig.width = 6, fig.asp = 0.618, out.width = "70%", fig.align = "center")
...
general_warning <- if (datatest == TRUE | sum(!is.na(htmldata$arsresultat)) == 0 | sum(!is.na(htmldata$sum_egenkapital_og_gjeld)) == 0 | (sum(!is.na(htmldata$lederlonn)) == 0 &
    sum(!is.na(htmldata$leder_annen_godtgjorelse)) == 0)) {
  "## Note:"
} else {
  ""
...
<!--# Need to decide whether to underline the KPIs, as in the first one -->

- **Fundraising ROI**: [`r fundraising`]{.underline}
...
```{r revenue_plot, fig.cap="Based on the category \`Revenue' in the Excel sheet."}
## Revenue--------------------------------------------------------------------
revenue_plot <-
...
```{r expenses_plot, fig.cap="Based on the category \`Expenses' in the Excel sheet."}
## Expenses -------------------------------------------------------------------
expenses_plot <-
  # testing for data quality
  if (datatest == TRUE) {
