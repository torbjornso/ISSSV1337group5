...
observeEvent(input$click, { # wait for button click
  ...
  output$KPI_box_private <- renderValueBox({
    valueBox(
      ...
      color = ifelse(KPI_private_num() < 100, "warning", "success"),
  ...
  output$KPI_box5 <- renderValueBox({
    valueBox(
      ...
      color = ifelse(KPI_public_num() < 100, "warning", "success"),
  ...
  # Reactive function with slightly nicer colours + ensuring positive values are
  # always green
  scale_fill_green <- reactive({
...
# Test version of function - multiple currencies
read_proff8 <- function(file, start, end) {
  read_html(file, encoding = "UTF-8") |>
    html_node("#inner-frame") |>
    html_table() |>
