...
- pygments, haddock, and kate are nice [code syntax highlighting alternatives](https://www.garrickadenbuie.com/blog/pandoc-syntax-highlighting-examples/) to the default.

- More elegant to use lines (***) around the code blocks and one of the non-default syntax highlight styles, I think. (See first two blocks for example.) (If output is supposed to be pdf.)
...
<!--# Might need to include tinytex? Or is that overkill? -- if (!require("tinytex") install.packages("tinytex"); tinytex::install_tinytex())-->
