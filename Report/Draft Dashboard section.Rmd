...
The sidebar includes file upload boxes for Excel and html files, numeric input boxes in which the user can input the years covered by the Proff.no data, and an 'Analyse' button which is connected to a Shiny `observe() |> bindEvent()` structure, ensuring that most of the code is not executed until the user has finished entering data.
...
The value box then also relies on a function that converts the number into a percentage string, checking into process if the number is `NA` or `Inf` (indicating a division by zero, and thus either a data problem or data not suited to this calculation).
```{r}
...
In the case of some value boxes, such as the Fundraising ROI one, the function includes logic that changes the colour of the box based on the calculated number. The levels are based on some cursory examination of expected levels of Fundraising ROI, but may of course be changed if the dashboard is developed further.
```{r}
...
All the boxes also include a Fontawesome icon, fetched using the fontawesome package, as Flexdashboard relies on the unsupported version 3 of Fontawesome, whilst fontawesome used the still-supported version 5 when coding began, and has just been updated to use version 6.1.1.
...
The 'no information' plot is an essentially empty plot displaying a notice to the user:
```{r}
...
Below these plots are two plots based on Excel data. These both use the same check for data quality as the value box, outputting a 'bad data' plot if the data are bad using a separate function:
```{r}
...
Finally, the dashboard contains a separate tab with some basic details about the KPIs and plots.
