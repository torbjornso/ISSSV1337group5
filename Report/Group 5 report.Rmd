---
title: "ISSSV1337 Group 5 Report"
author:    | 
    | Amalie Halle Christensen, Ida Kristine Garthe
    | Torbjørn Skinnemoen Ottersen, Henrik Waage Rui
    | Agnieszka Sadlowska
date: "2022-08-03"
output:
  pdf_document: 
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, error = FALSE, message = FALSE, out.width = "70%")
```
...
**Github**: [https://github.com/torbjornso/ISSSV1337group5](https://github.com/torbjornso/ISSSV1337group5)
...
description and findings are presented. Lastly, we discuss limitations and steps
forward, and provide a brief summary of our experience.


## Problem statement

The project assigned to our group was Oslo Philanthropic Exchange (OPX), 
a startup working to develop solutions to assist individuals and 
organisations wanting to donate to charities and designing a socially 
responsible investment strategy. Their main goal is to increase cash 
flow from private groups and individuals to charitable purposes. Therefore, the donors' trust in the charities’ abilities to carry out their promises is of paramount significance. 
 
To do this, data from charities need to be collected. One of OPX’s current 
challenges is data acquisition, as data currently must be collected manually. They therefore wanted a way to automate data collection, for example using web scraping.
 

Additionally, by creating Key Performance Indicators based on these data, OPX can use them to benchmark the relationship between financial assets and emergency relief obligations. 
...
2. Creating KPI(s) is important for the organisation’s performance 
management, to influence its objectives, and to foster growth.
...
Our brainstorming process for this case was characterised by several
...
points. Therefore, we have decided to divide the brainstorming section into
three phases.

## Phase 1: How to harvest and analyse the data?
...
We discussed creating web scraping code that could collect data across
...
suggested solution was to create code that could read PDF documents
that OPX downloaded themselves. Our code would analyse and organise the
uploaded data from the PDF documents.

However, different organisations publish financial data in different
ways in their annual reports -- some financial statements were included
as tables, some as pictures, etc. This led us to phase 2.

<!-- # All images need captions-->

```{r, echo=FALSE, fig.cap="Norwegian Cancer Society 2021: The financial report saved as text in the PDF."}
# Illustration some text some pictures in financial statements
knitr::include_graphics("Images/lagret_tekst.png") # Kreftforeningen 2021
```

```{r, echo=FALSE, fig.cap="Rainforest Foundation 2018: The financial report saved as an image in the PDF."}
# Illustration some text some pictures in financial statements
knitr::include_graphics("Images/lagret_bilde.png") # Regnskogfondet 2018
```

## Phase 2: Exploring solutions

At this stage, we wanted to explore alternatives to analysing PDF
documents. We discussed how to do so, and concluded that we had three
options:

- Financial statements in anual report PDF documents. 
- Web scrape Proff.no for financial statements. 
- Web scrape the Brønnøysund registry.
...
Also, we quickly found that Proff.no disallowed web scraping on their
website. The Brønnøysund registry did offer API access to all the financial statements on their website, but we discovered that
access to these data costs about half a million NOK.

We started to look at workarounds for this problem. It would still be
possible to download Proff.no HTML web pages manually for each
organisation. We also received access to some XML files from Statistics Norway, but this would not be a viable solution for OPX in the long term.
...
into R. While doing so, we realised that it would be practically
...
```{r, echo=FALSE, fig.cap="Accounting items for the Norwegian Cancer Society 2020 and the Norwegian Diabetes Association 2020, gathered from the Brønnøysund registry.", fig.align='center'}
# Illustration brns vs OPX accounting items
knitr::include_graphics("Images/Brns_items1.png")
```

```{r, echo=FALSE, fig.cap="Accounting items for the Norwegian Cancer Society 2020 and the Norwegian Diabetes Association 2020, gathered from OPX.", fig.align='center'}
# Illustration brns vs OPX accounting items
knitr::include_graphics("Images/OPX_items1.png")
```

```{r, echo=FALSE, fig.cap="An attempt at marrying the two sets of accounting items.", fig.align='center'}
# Illustration brns vs OPX accounting items
knitr::include_graphics("Images/Marriage.png")
```

When we also discovered that not all organisations publish their financial
...
```{r, echo=FALSE, out.width="100%", fig.cap="The Rainforest Foundation stopped posting financial statement PDFs on their website in 2018."}
# Illustration only published for some years example
knitr::include_graphics("Images/Regnskogfondet_stops2018.png")
```

```{r, echo=FALSE, out.width="50%", fig.align='center', fig.cap="The Norwegian Cancer Society started posting financial statement PDFs on their website in 2019."}
# Illustration only published for some years example
knitr::include_graphics("Images/Kreftforeningen_starts2019.png")
```

## Phase 3: Scrapping the scraping

The OPX case requested two tasks -- web scrape financial data, and create
...
also decided to create an interactive dashboard that OPX could use, which 
would display the KPIs as well as financial trends for the selected
organization. To use this, OPX needs to gather the financial data
themselves, then upload this into the shiny app as an excel and an 
html document.

# Data collection

We wanted to use web scraping to collect the data, so that we could have
...
been imported from XML files. However, once we realised that the
...
organisations and organised into useful accounting items.

We copied these financial statements manually into an Excel document.
Perhaps a 'simple' solution, however our code requires OPX to upload
both an Excel document, as well as a HTML document
from Proff.no, to the dashboard.

```{r, echo=FALSE, out.width="100%", fig.cap="The data format used to upload to the dashboard."}
# Bilde av den "endelige" excel-versjonen som brukes til å laste inn i shiny
knitr::include_graphics("Images/Excel_final.png")
```
...
Our data was gathered from OPX's own web page, and from Proff.no. As described 
in the previous parts, we wanted to gather data from the annual reports or by 
scraping the Brønnøyund registry or Proff.no, but found this to be impossible. The data used is the one that OPX has gathered themselves directly 
from the organisations. This data was also used to test that our codes in R 
were correct.

We have also used Proff.no to visualise the annual incomes and some KPIs. 
This is data that the organisations have reported themselves to Proff.no. 
Since there are no regulations regarding reporting data for NGOs in Norway, 
the organisations have not been consistent with the data reporting. This may 
explain the lacunae in the presented data (see e.g. figures 9 and 10).

```{r arsresultat, echo=FALSE, fig.align='center', fig.asp=0.5, fig.cap="Net income/loss (in NOK 1,000) data for the Norwegian Cancer Society from Proff.no"}
kreftforeningen |>
  rename(
    Year = year,
...
```{r care_arsresultat, echo=FALSE, fig.align='center', fig.asp=0.5, fig.cap="Net income/loss (in NOK 1,000) data for CARE Norway from Proff.no"}
care |>
  rename(
    Year = year,
    `Net income` = arsresultat,
...
Whether this is a true measure of fundraising ROI depends on how the underlying data have been calculated -- e.g. if they include salary costs, etc.
...
This KPI may say something about the NGO's sources of funding. There is *prima facie* no specific desirable funding mix, but this measure may be of interest to some donors.
...
As the dashboard uses a fair number of packages, it uses the pacman package to ensure they are installed, to avoid the user being confronted with (cryptic) messages about missing functions when running the dashboard locally:^[A test version uploaded to [shinyapps.io](https://www.shinyapps.io) uses normal serial library calls, as the server then automatically installs the necessary packages during deployment.]
...
The sidebar includes file upload boxes for Excel and html files, numeric input boxes in which the user can input the years covered by the Proff.no data (needed because of the highly variable availability of historical data), and an 'Analyse' button which is connected to two Shiny `observe() |> bindEvent()` structures, ensuring that most of the code is not executed until the user has finished entering data. (Though both structures await the same button click, the division was necessary to remain within R's 10,000 byte limit.)
...
```{r dashboard_images, out.width="50%", fig.cap="The dashboard.", fig.subcap=c("The main page.", "The details tab."), echo=FALSE}
...
```{r dashboard_report_image, echo=FALSE, out.width="50%", fig.align='center', fig.cap="Sample dashboard report."}
knitr::include_graphics("Images/Dashboard_report_cropped.png")
```
...
`r if (knitr::is_latex_output()) line`
```{r dashboard_download, eval=FALSE}
...
`r if (knitr::is_latex_output()) line`
```{r dashboard_download_button, eval=FALSE}
## Download button -----------------------------------------------------------
...
of relevant data in the annual reports from the NGOs. Since there is often 
no obligation to provide annual accounts, they often have more information 
about the organisation in general rather than numbers. If they do provide 
...
in an image format rather than text. The lack of data, standardisation and 
diverse formats made it impossible to create an automated solution for (safely)
...
experimented with an annual report from Brønnøysund. Although 
the reports were standardised, the different posts in the report did not 
...
alternatives, we concluded that the NGOs could report in a standardised 
...
due to OPX’s need for this data to be available, we weould suggest discussions 
with relevant politicians and ministry officials as a starting point.

Based on the findings of working with this case, we would recommend looking at best practices for further development. One well-known established actor in the field is Charity Navigator.

# Our experience

In this last section, we would like to describe how we experienced this process and what we have learned. As explained, the main challenge with the task was thatb it was in many ways unsolvable due to non-existent data. In cases where data were available, they were inadequate.
...
The lectures allowed us to learn about many interesting topics. However, as the connection between what we learned there and what we got to utilise in the case was distant, and parts were too advanced for some of us, especially those without any particular experience within statistics.

Despite this, we have done our best to work with the resources at hand. As shown in this presentation, trying out solutions and later scrapping them has been a big part of the problem-solving process. We also got an insight into real-life work situations, such as the challenges that come when what is desired clashes with what is possible. We are certain that these experiences will be useful for all of us in the future, and we are grateful for being given this opportunity.
