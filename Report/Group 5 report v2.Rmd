...
<!--# Vi burde vel ha navnene våre, siden rapporten skal lastes opp på Inspera?-->

<!--# Legg til et sted: The Norwegian Control Committee for Fundraising (Innsamlingskontrollen) also has some very basic financial data and information available about selected charitable organisations.-->
...
flow from private groups and individuals to charitable purposes. Therefore, the donor's trust 
in the charities’ abilities to carry out their promises is of paramount significance. 
...
challenges is data acquisition, as data currently must be collected manually. They therefore wanted a way to automate data collection, for example using web scraping.
 ...
 <!-- Needs work -->
Additionally, by creating Key Performance Indicators based on these data, OPX can use 
...
## Phase 1: How to harvest and analyse the data? {.smaller}
...
```{r, echo=FALSE, fig.cap="Kreftforeningen 2021: The financial report saved as text in the PDF"}
...
```{r, echo=FALSE, fig.cap="Regnskogfondet 2018: The financial report saved as an image in the PDF"}
# Illustration some text some pictures in financial statements
...
```{r, echo=FALSE, fig.cap="Accounting items for Kreftforeningen 2020 and Diabetesforbunet 2020, gathered from the Brønnøysund registry", fig.align='center'}
# Illustration brns vs OPX accounting items
knitr::include_graphics("Images/Brns_items1.png")
```

```{r, echo=FALSE, fig.cap="Accounting items for Kreftforeningen 2020 and Diabetesforbunet 2020, gathered from OPX", fig.align='center'}
# Illustration brns vs OPX accounting items
...
```{r, echo=FALSE, out.width="100%", fig.cap="Regnskogfondet stopped posting financial statement PDFs on their website in 2018"}
# Illustration only published for some years example
knitr::include_graphics("Images/Regnskogfondet_stops2018.png")
```

```{r, echo=FALSE, out.width="100%", fig.cap="Kreftforeningen started posting financial statement PDFs on their website in 2019"}
# Illustration only published for some years example
...
```{r, echo=FALSE, out.width="100%", fig.cap="The data format used to upload into shiny"}
...
Our data was gathered from OPXs own web page, and from Proff.no. As described 
in the previous parts, we wanted to gather data from the yearly reports or by 
scraping the Brønnøyund registry or Proff.no, but found this to be close to 
impossible. The data used is the one that OPX has gathered themselves directly 
from the organizations. This data was also used to test that our codes in R 
were correct.

We have also used Proff.no to visualize the annual incomes and some KPIs. 
This is data that the organizations have reported themselves to Proff.no. 
Since there are no regulations regarding reporting data for NGOs in Norway, 
the organizations have not been consistent with the data reporting. This 
explains the holes in the presented data.

<!--# Fra Visualisations (nedenfor): The plots based on data from Proff.no often demonstrate surprising lacunae, but it difficult to know where they originate -- NGO reporting, Brønnøysund, or Proff.no.-->

<!--Legg inn bilder her-->

<!-- # Legger inn fire grafer som viser manglende data i Proffs årsregnskap-->
