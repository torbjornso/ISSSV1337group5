...
OPX wanted us to harvest the organisations' financial data from the web.
...
difficult, and that it would only work with selected organisations. The next
...
```{r, echo=FALSE, fig.cap="Need caption"}
...
the issue of financial statements not always being published as text, as well as their often wildly differing formats. Unfortunately, we quickly found that Proff.no disallows any automated collection of data from their website. The Brønnøysund registry does offer API access to all the financial statements, but we discovered that access currently costs NOK 480,000.

We started to look at workarounds for this problem. It could still be
possible to download Proff.no HTML websites manually for each
organisation. We did receive some XML files from Statistics Norway, but using these is ultimately not a viable solution for OPX.
...
```{r, echo=FALSE, fig.cap="Need caption", fig.align='center'}
knitr::include_graphics("Images/info_kreft.png")
```


```{r, echo=FALSE, out.width="100%", fig.cap="Need caption"}
knitr::include_graphics("Images/regnskapssti.png")
```

When also discovering that not all organisations publish their financial
statements, and certainly not for every year, we decided to give up on
collecting financial data for OPX, as the task turned out to impossible given current reporting requirements and practices. This is when we moved on to the third
and final stage.
...
also decided to create an interactive dashboard based on Shiny that OPX could use, which would
display the KPIs as well as financial trends for the selected
organisation. To use this, OPX needs to gather the financial data
themselves, as well as download data manually from Proff.no, and then upload this into the dashboard in the form of Excel and HTML documents.
...
been imported from XML files. However, once we realised that the available
...
```{r egenkapital, echo=FALSE, fig.align='center', fig.asp=0.5, fig.cap="Total equity/liabilities (in NOK 1,000) data for the Norwegian Cancer Society from Proff.no"}
kreftforeningen |>
...
```{r CEO, echo=FALSE, fig.align='center', fig.asp=0.618, fig.cap="CEO compensation data for the Norwegian Cancer Society from Proff.no"}
kreftforeningen |>
