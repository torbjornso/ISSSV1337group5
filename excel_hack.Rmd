...
>Du har ikke lastet opp det nye Excel-arket :) (Plasser det gjerne i Data-mappen ;)) --Torbjørn

>Nå er arket lastet opp i Data-mappen
--Aga
...
jukseark <- read_excel("./Data/OPXfinansAGA.xlsx", col_names = c("entry", "value"))
...
juksedata <- jukseark %>%
  ...
  mutate_at(vars(-org_name), as.numeric) %>%
  
  # Legger til en kolonne med KPI
  mutate(KPI_ROI = fundraising/(private_contributions+membership_fees))
```
...
juksedata_1 <- jukseark_1 %>% 
  pivot_wider(names_from = Name, values_from = Value) %>%
  clean_names() %>% 
  mutate(KPI_ROI = undraising/(private_contributions+membership_fees), 
         na.rm = TRUE)
