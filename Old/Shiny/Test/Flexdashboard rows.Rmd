...
observe({ # wait for button click
        ...
        transmute(
          ...
          KPI_fundraising_num =
            (sum(revenue_private_contributions,
              revenue_membership_fees, na.rm = TRUE) - expenses_fundraising) /
  ...
  # For some bizarre reason I cannot do na.rm = TRUE in these sum
  ...
  OPX_num <- reactive({
    ...
    if (datatest() == TRUE) {
      ...
      exceldata() |>
        ...
        transmute(
          KPI_OPX =
            sum(revenue_private_contributions, revenue_investments, assets_cash_in_bank_and_cash_equivalents) /
            sum(liability_short_term_grants_payable, liability_revocable_endowments, liability_long_term_grants_payable) * 100
        ) |>
  ...
  output$KPI_box_OPX <- renderValueBox({
    valueBox(
      value = KPI_to_string(OPX_num()),
      caption = "Liquid assets / obligations",
      color = if (is.na(OPX_num())) {
        "primary"
      } else if (OPX_num() < 100) {
        "warning"
      } else {
